...
def _free_user(uid="u1", days_inactive=155, notify=True, archived=False) -> User:
    return User(
        ...
        notify_account_deletion=notify,
...
async def test_warning_sent_to_eligible_user(mocker):
    """Warning email sent to eligible free-tier user."""
    user = _free_user()
    db = _db_for_warning([user])
    email = AsyncMock()
    email.send_account_deletion_warning = AsyncMock(return_value=True)
...
async def test_warning_skipped_for_admin(mocker):
    """Admin user is never sent a warning email."""
...
async def test_warning_deduplicated(mocker):
    """Warning not sent if already sent within the window."""
    user = _free_user()
...
def _db_for_purge(users, src_count=3) -> AsyncMock:
    ...
    user_r.all.return_value = users
    cnt_r = MagicMock()
    cnt_r.one.return_value = src_count
...
async def test_purge_archives_user_and_clears_credits(mocker):
    """Successful purge sets is_archived=True and credits_balance=0."""
...
async def test_purge_skips_admin(mocker):
    """Admin user is never purged."""
    user = _free_user(uid="admin2", days_inactive=200)
    db = _db_for_purge([user])
    deletion = MagicMock()
    deletion.delete_storage_by_tenant = AsyncMock()
