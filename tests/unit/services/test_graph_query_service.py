...
async def test_execute_nl_query_happy_path_returns_all_fields() -> None:
    ...
    rows = [{
        ...
        "r_type": None, "r_props": None, "m_type": None, "m_props": None,
    }]
    svc = GraphQueryService(llm_router=_make_mock_llm(_SAFE_CYPHER), neo4j_adapter=_make_mock_neo4j(rows))
    result = await svc.execute_nl_query(query="Show my memories", tenant_id=TEST_TENANT_ID, profile_id=None, limit=20)
...
async def test_execute_nl_query_unsafe_cypher_raises_http422() -> None:
    ...
    svc = GraphQueryService(llm_router=_make_mock_llm("CREATE (n:Memory {content: 'injected'})"), neo4j_adapter=mock_neo4j)
    with pytest.raises(HTTPException) as exc_info:
        await svc.execute_nl_query(query="Delete all", tenant_id=TEST_TENANT_ID, profile_id=None, limit=20)
...
async def test_execute_nl_query_empty_results_explanation() -> None:
    from src.services.graph_query_service import GraphQueryService  # noqa: PLC0415
    svc = GraphQueryService(llm_router=_make_mock_llm(_SAFE_CYPHER), neo4j_adapter=_make_mock_neo4j([]))
    result = await svc.execute_nl_query(query="Find projects", tenant_id=TEST_TENANT_ID, profile_id=None, limit=20)
...
async def test_execute_nl_query_llm_task_is_cypher_generation() -> None:
    ...
    await svc.execute_nl_query(query="My Python skills", tenant_id=TEST_TENANT_ID, profile_id=None, limit=20)
...
async def test_execute_nl_query_user_query_in_xml_tags() -> None:
    ...
    await svc.execute_nl_query(query=test_query, tenant_id=TEST_TENANT_ID, profile_id=None, limit=20)
...
async def test_execute_nl_query_query_time_ms_non_negative_int() -> None:
    from src.services.graph_query_service import GraphQueryService  # noqa: PLC0415
    svc = GraphQueryService(llm_router=_make_mock_llm(_SAFE_CYPHER), neo4j_adapter=_make_mock_neo4j([]))
    result = await svc.execute_nl_query(query="test", tenant_id=TEST_TENANT_ID, profile_id=None, limit=20)
