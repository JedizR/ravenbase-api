...
async def test_graph_extraction_returns_ok_on_success(arq_ctx) -> None:
    ...
    with patch("src.workers.graph_tasks.GraphService") as mock_service:
        mock_service.return_value.extract_and_write = AsyncMock(return_value=mock_stats)
...
async def test_graph_extraction_returns_error_on_service_failure(arq_ctx) -> None:
    ...
    with patch("src.workers.graph_tasks.GraphService") as mock_service:
        mock_service.return_value.extract_and_write = AsyncMock(
